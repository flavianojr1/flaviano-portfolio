    ...
    <main className="grid-bg min-h-screen overflow-hidden transition-colors duration-300">
          ...
          {p.projectsData.map((x) => (
            ...
            >
              ...
              <div className="relative min-h-[240px] w-full overflow-hidden bg-slate-100 sm:min-h-[300px] md:min-h-full dark:bg-[#07111f]">
                <img
                  src={x.image}
                  alt={`Prévia do projeto ${t(x.title)}`}
              ...
              <div className="flex flex-col justify-center p-6 sm:p-8 lg:p-9">
                ...
                <h3 className="mt-2 text-xl font-bold text-slate-900 dark:text-white sm:text-2xl">{t(x.title)}</h3>
                ...
                <div className="mt-5 flex items-center justify-between border-t border-slate-200 pt-3.5 dark:border-white/10">
                  ...
                  {(x as any).liveUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      aria-label={`Acessar projeto ${t(x.title)}`}
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      aria-label={`Ver repositório do projeto ${t(x.title)}`}
                      className="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-3.5 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-slate-800 dark:bg-white dark:text-ink dark:hover:bg-slate-100"
                    >
                      <Github size={13} />
                      <span>{locale === 'pt' ? 'Repositório' : 'GitHub'}</span>
                    </a>
                  )}
                </div>
              </div>
      ...
      <Section id="experiencia">
        ...
        <div className="mt-10 space-y-6">
          {p.experienceData.map((e) => (
            <motion.article
              whileHover={{ y: -3 }}
              key={`${e.company}-${e.period}`}
              className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-sky-500/30 hover:shadow-md dark:border-white/10 dark:bg-white/[.03] dark:hover:border-cyan/30 sm:p-8"
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-xs font-semibold text-sky-600 dark:text-cyan">
                    {t(e.role)}
                  </p>
                </div>
              ...
              <ul className="mt-5 space-y-2 text-xs leading-6 text-slate-600 dark:text-slate-400">
                {t(e.highlights).map((h: string) => (
                  <li key={h} className="flex items-start gap-2.5">
                    <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-sky-600 dark:bg-cyan" />
                    <span>{h}</span>
                  </li>
                ))}
              </ul>
      ...
      <Section id="formacao">
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3">
          {p.educationData.map((e) => (
            <div
              key={e.institution}
              className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[.03]"
            >
              <div>
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-bold text-sky-600 dark:text-cyan">{e.period}</span>
                  <span className="rounded-full bg-sky-100 px-2.5 py-0.5 text-[11px] font-semibold text-sky-800 dark:bg-cyan/10 dark:text-cyan">
                    {t(e.status)}
                  </span>
                </div>
                <h3 className="mt-4 text-base font-bold text-slate-900 dark:text-white">{e.institution}</h3>
                <p className="mt-2 text-xs leading-5 text-slate-600 dark:text-slate-400">{t(e.course)}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 border-t border-slate-200/80 pt-8 dark:border-white/10">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            ...
            <span className="font-mono text-[11px] text-slate-500 dark:text-slate-400">
              {t(p.skillsCloud).tags.length} {locale === 'pt' ? 'competências' : 'skills'}
            </span>
          </div>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {t(p.skillsCloud).tags.map((tag: string) => (
              <span
                key={tag}
